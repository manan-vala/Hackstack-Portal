    ...
    <div className="flex items-end justify-center gap-3 pt-8 pb-0 px-4">
      {PODIUM_ORDER.map((dataIdx, colIdx) => {
        ...
        return (
          ...
          >
            {/* Avatar + name */}
            <Avatar username={entry.username} avatarUrl={entry.avatarUrl} size={isFirst ? "lg" : "md"} rank={rank} />
            <span className={`font-semibold text-gray-800 dark:text-gray-100 ${isFirst ? "text-sm" : "text-xs"} text-center max-w-[80px] truncate`}>
              {entry.username}
            </span>
            ...
            <div
              className={`w-24 ${HEIGHTS[colIdx]} rounded-t-lg flex flex-col items-center justify-center gap-1
                ${rank === 1 ? "bg-yellow-100 dark:bg-yellow-900/40 border border-yellow-300 dark:border-yellow-700" :
                  rank === 2 ? "bg-gray-100  dark:bg-gray-800  border border-gray-300  dark:border-gray-600"  :
