...
export default function LeaderboardRow({ entry, index, isCurrentUser }) {
  const badgeClass = RANK_BADGE[entry.rank] ?? "bg-gray-50 text-gray-500 dark:bg-gray-800 dark:text-gray-400";
  const MEDALS = { 1: "🥇", 2: "🥈", 3: "🥉" };
  ...
  return (
    ...
    >
      {/* Rank badge */}
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold flex-shrink-0 ${badgeClass}`}>
        {MEDALS[entry.rank] ?? `#${entry.rank}`}
      </div>

      {/* User */}
      <div className="flex items-center gap-2.5 min-w-0">
        <Avatar username={entry.username} avatarUrl={entry.avatarUrl} size="sm" rank={entry.rank} />
      ...
      <div className="text-center hidden sm:block">
        <div className="text-sm font-semibold text-gray-800 dark:text-gray-100">{entry.modulesCompleted}</div>
        <div className="text-[10px] text-gray-400 dark:text-gray-500">modules</div>
      </div>
