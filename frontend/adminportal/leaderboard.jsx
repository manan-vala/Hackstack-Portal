...
import { useLeaderboard, useModules } from "../hooks/useLeaderboard";
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {myEntry && myEntry.rank > 3 && (
          <div className="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-3 mb-6">
            <Avatar username={myEntry.username} avatarUrl={myEntry.avatarUrl} size="sm" />
            <div className="flex-1">
              <p className="text-xs text-blue-500 dark:text-blue-400 font-medium">Your rank</p>
              <p className="text-sm font-bold text-blue-800 dark:text-blue-200">
                #{myEntry.rank} — {myEntry.totalPoints.toLocaleString()} pts
              </p>
            </div>
            <div className="text-xs text-blue-500 dark:text-blue-400 font-medium">
              {myEntry.modulesCompleted} module{myEntry.modulesCompleted !== 1 ? "s" : ""} done
            </div>
          </div>
        )}
        ...
        <div className="flex gap-1.5 flex-wrap bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-1 mb-6">
          ...
          {!modulesLoading && modules.map((m) => (
            <TabButton key={m._id} active={view === m.slug} onClick={() => setView(m.slug)}>
              {m.title}
            </TabButton>
          ))}
        </div>
            ...
            <motion.div key={view} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div className="grid grid-cols-3 divide-x divide-gray-100 dark:divide-gray-800 text-center py-3">
                    {top3.map((e) => (
                      <div key={e.userId} className="px-2">
                        <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{e.username}</p>
                        <p className="text-sm font-bold text-gray-800 dark:text-gray-100">{e.totalPoints.toLocaleString()} pts</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {rest.length > 0 && (
                <div className="flex items-center gap-2 my-3 px-1">
                  ...
                  <span className="text-[11px] text-gray-400 dark:text-gray-500 font-medium">Remaining</span>
                  <div className="flex-1 h-px bg-gray-100 dark:bg-gray-800" />
                </div>
              )}
